import { TILE_SIZE, MATRIX_EFFECT_DURATION, CharacterState, Direction, PetKind } from '../types'
import {
  HUE_SHIFT_MIN_DEG,
  HUE_SHIFT_RANGE_DEG,
...
import type { Character, Seat, FurnitureInstance, TileType as TileTypeVal, OfficeLayout, OfficePet, PlacedFurniture } from '../types'
...
const GATEWAY_SRE_CHARACTER_VARIANT = 3
...
export class OfficeState {
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const ch of this.characters.values()) {
      if (ch.isCat || ch.isDog || ch.isLobster || ch.isSystemRole) continue
  ...
  private getDefaultPetCharacterId(petId: string): number | null {
    if (petId === 'pet-cat-1') return OfficeState.CAT_ID
    if (petId === 'pet-lobster-1') return OfficeState.LOBSTER_ID
    if (petId === 'pet-lobster-2') return OfficeState.HUNTER_LOBSTER_ID
    return null
  }
  ...
  private syncPetsFromLayout(): void {
    ...
    for (const [petId, characterId] of this.petCharacterIds) {
      if (livePetIds.has(petId)) continue
    ...
    for (const pet of pets) {
      let characterId = this.petCharacterIds.get(pet.id)
      if (characterId === undefined) {
        characterId = this.getDefaultPetCharacterId(pet.id) ?? this.nextPetCharacterId--
        this.petCharacterIds.set(pet.id, characterId)
      ...
      if (existing) {
        ...
        existing.palette = this.getPetSkin(pet)
        existing.hueShift = 0
      ...
      const ch = createCharacter(characterId, 0, null, null, 0)
      ch.isActive = false
      ch.state = CharacterState.IDLE
      ch.wanderTimer = 1 + Math.random() * 3
      this.applyPetKind(ch, pet.kind)
      ch.palette = this.getPetSkin(pet)
      ch.hueShift = 0
      this.applyPetColor(ch, pet)
      ch.label = pet.name ?? ''
      this.placePetCharacter(ch, pet)
      this.characters.set(characterId, ch)
    }
  }

  private getPetSkin(pet: OfficePet): number {
    return pet.kind === PetKind.CAT && typeof pet.skin === 'number' && Number.isInteger(pet.skin) && pet.skin >= 0 ? pet.skin : 0
  }

  private applyPetColor(ch: Character, pet: OfficePet): void {
    ch.petColor = (pet.kind === PetKind.CAT || pet.kind === PetKind.DOG) && pet.color
      ? { ...pet.color }
      : undefined
  }

  private applyPetKind(ch: Character, kind: OfficePet['kind']): void {
    ch.isCat = kind === PetKind.CAT
    ch.isDog = kind === PetKind.DOG
    ch.isLobster = kind === PetKind.LOBSTER
    if (!ch.isLobster) {
      ch.lobsterRageTimer = 0
      ch.lobsterBubbles = []
    }
  }

  private placePetCharacter(ch: Character, pet: OfficePet): void {
    ch.tileCol = pet.col
    ch.tileRow = pet.row
    ch.x = pet.col * TILE_SIZE + TILE_SIZE / 2
    ch.y = pet.row * TILE_SIZE + TILE_SIZE / 2
    ch.path = []
    ch.moveProgress = 0
    ch.seatId = null
  }

  getPetCharacterId(petId: string): number | null {
    return this.petCharacterIds.get(petId) ?? null
  }

  getPetIdForCharacterId(characterId: number): string | null {
    return this.petIdsByCharacterId.get(characterId) ?? null
  }
  ...
  ensureGatewaySre(): void {
    ...
    if (existing) {
      existing.palette = GATEWAY_SRE_CHARACTER_VARIANT
      existing.hueShift = 0
      return
    }
    const spawn = this.findClosestWalkable(GATEWAY_SRE_STANDBY_COL, GATEWAY_SRE_STANDBY_ROW)
    const ch = createCharacter(id, GATEWAY_SRE_CHARACTER_VARIANT, null, null, 0)
  ...
  private getFirstIdleHumanoid(): Character | null {
    const list = Array.from(this.characters.values())
      .filter(ch => !ch.isCat && !ch.isDog && !ch.isLobster && !ch.isSystemRole && ch.state === CharacterState.IDLE && ch.matrixEffect !== 'despawn')
      .sort((a, b) => a.id - b.id)
    return list[0] || null
  }
  ...
  pushCodeSnippet(id: number, text: string): void {
    const ch = this.characters.get(id)
    if (!ch || ch.isCat || ch.isDog || ch.isLobster) return
  ...
  private resolveWalkConflicts(): void {
    ...
    for (const ch of this.characters.values()) {
      if (ch.matrixEffect || ch.isCat || ch.isDog || ch.isLobster || ch.greetLocked) continue
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.isViewingPhoto && ch.state === CharacterState.IDLE && !ch.isCat && !ch.isDog && !ch.isLobster) {
      ...
      } else if (ch.isActive && ch.state === CharacterState.TYPE && !ch.isCat && !ch.isDog && !ch.isLobster && !ch.isSubagent) {
  ...
  getPetAt(worldX: number, worldY: number): number | null {
    ...
    for (const ch of chars) {
      ...
      const dy = worldY - (ch.y + (ch.isLobster ? 2 : -4))
      const hitR = ch.isLobster ? LOBSTER_HIT_RADIUS_PX : 10
      if (dx * dx + dy * dy <= hitR * hitR) return ch.id
    }
    return null
  }
}
