...
export interface FurnitureOcclusionBounds {
  x: number
  y: number
  w: number
  h: number
...
export interface FurnitureInstance {
  ...
  emojiScale?: number
  /** Runtime bounds used for wall occlusion repair; usually excludes background rows. */
  wallOcclusionBounds?: FurnitureOcclusionBounds
  /** True for wall-mounted/background-only items that should not be ghosted through walls. */
  wallOcclusionDisabled?: boolean
}
...
export interface OfficePet {
  ...
  col: number
  row: number
  /** Cat skin index; omitted or 0 means default cat. */
  skin?: number
  /** Optional per-pet color tint for cats and dogs. */
  color?: FloorColor
  /** Display name shown as a label above the pet */
  name?: string
}
