import { TileType, MAX_COLS, MAX_ROWS, PetKind } from '../types'
import { DEFAULT_NEUTRAL_COLOR } from '../constants'
import type { TileType as TileTypeVal, OfficeLayout, OfficePet, PetKind as PetKindType, PlacedFurniture, FloorColor } from '../types'
...
export function canPlacePet(layout: OfficeLayout, col: number, row: number, excludeId?: string): boolean {
  ...
  return !(layout.pets ?? []).some((p) => p.id !== excludeId && p.col === col && p.row === row)
}

function normalizePetName(name?: string): string | undefined {
  const trimmed = (name ?? '').trim()
  return trimmed.length > 0 ? trimmed : undefined
}

function normalizePetSkin(skin?: number): number {
  return typeof skin === 'number' && Number.isInteger(skin) && skin >= 0 ? skin : 0
}

function supportsPetColor(kind: PetKindType): boolean {
  return kind === PetKind.CAT || kind === PetKind.DOG
}

function normalizePetColor(color?: FloorColor | null): FloorColor | undefined {
  if (!color) return undefined
  const { h, s, b, c } = color
  if (![h, s, b, c].every(Number.isFinite)) return undefined
  return {
    h: Math.round(h),
    s: Math.round(s),
    b: Math.round(b),
    c: Math.round(c),
    ...(color.colorize ? { colorize: true } : {}),
  }
}

function petSkinPatch(kind: PetKindType, skin?: number): Pick<OfficePet, 'skin'> {
  const normalizedSkin = normalizePetSkin(skin)
  return kind === PetKind.CAT && normalizedSkin > 0 ? { skin: normalizedSkin } : {}
}

function petColorPatch(kind: PetKindType, color?: FloorColor | null): Pick<OfficePet, 'color'> {
  const normalizedColor = normalizePetColor(color)
  return supportsPetColor(kind) && normalizedColor ? { color: normalizedColor } : {}
}

function samePetColor(a?: FloorColor, b?: FloorColor): boolean {
  if (!a && !b) return true
  if (!a || !b) return false
...
export function addPet(
  ...
  if (!canPlacePet(layout, col, row)) return layout
  ...
  const pet: OfficePet = {
    ...
    col,
    row,
    ...petSkinPatch(kind, skin),
    ...petColorPatch(kind, color),
    ...(normalizedName ? { name: normalizedName } : {}),
  }
  return { ...layout, pets: [...(layout.pets ?? []), pet] }
}

export function movePet(layout: OfficeLayout, id: string, col: number, row: number): OfficeLayout {
  if (!canPlacePet(layout, col, row, id)) return layout
  const pets = layout.pets ?? []
  const pet = pets.find((p) => p.id === id)
  if (!pet || (pet.col === col && pet.row === row)) return layout
  return { ...layout, pets: pets.map((p) => (p.id === id ? { ...p, col, row } : p)) }
}

export function removePet(layout: OfficeLayout, id: string): OfficeLayout {
  const pets = layout.pets ?? []
  const filtered = pets.filter((p) => p.id !== id)
  if (filtered.length === pets.length) return layout
  return { ...layout, pets: filtered }
}

export function changePetKind(layout: OfficeLayout, id: string, kind: PetKindType, skin?: number): OfficeLayout {
  const pets = layout.pets ?? []
  const pet = pets.find((p) => p.id === id)
  const normalizedSkin = normalizePetSkin(skin)
  if (!pet || (pet.kind === kind && (pet.skin ?? 0) === normalizedSkin)) return layout
  return {
    ...layout,
    pets: pets.map((p) => {
      if (p.id !== id) return p
      const { skin: _skin, color, ...petWithoutSkin } = p
      return { ...petWithoutSkin, kind, ...petSkinPatch(kind, normalizedSkin), ...petColorPatch(kind, color) }
    }),
  }
}

export function recolorPet(layout: OfficeLayout, id: string, color: FloorColor | null): OfficeLayout {
  const pets = layout.pets ?? []
  const pet = pets.find((p) => p.id === id)
  if (!pet || !supportsPetColor(pet.kind)) return layout
  const normalizedColor = normalizePetColor(color)
  if (samePetColor(pet.color, normalizedColor)) return layout
  return {
    ...layout,
    pets: pets.map((p) => {
      if (p.id !== id) return p
      const { color: _color, ...petWithoutColor } = p
      return normalizedColor ? { ...petWithoutColor, color: normalizedColor } : petWithoutColor
    }),
  }
}

export function renamePet(layout: OfficeLayout, id: string, name: string): OfficeLayout {
  const pets = layout.pets ?? []
  const pet = pets.find((p) => p.id === id)
  const normalizedName = normalizePetName(name)
  if (!pet || (pet.name ?? '') === (normalizedName ?? '')) return layout
  return {
    ...layout,
    pets: pets.map((p) => {
      if (p.id !== id) return p
      const { name: _name, ...petWithoutName } = p
      return normalizedName ? { ...petWithoutName, name: normalizedName } : petWithoutName
    }),
  }
}
...
export function expandLayout(
  ...
  const newPets = layout.pets?.map((p) => ({
    ...p,
    col: p.col + shiftCol,
    row: p.row + shiftRow,
  }))
  ...
  return {
    layout: { ...layout, cols: newCols, rows: newRows, tiles: newTiles, tileColors: newColors, furniture: newFurniture, ...(newPets ? { pets: newPets } : {}) },
